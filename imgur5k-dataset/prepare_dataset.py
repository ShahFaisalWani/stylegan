...
def parse_bounding_box(bbox_string):
    # Remove any whitespace and ensure the string is enclosed in square brackets
    bbox_string = bbox_string.strip()
    if not bbox_string.startswith('['):
        bbox_string = '[' + bbox_string
    if not bbox_string.endswith(']'):
        bbox_string = bbox_string + ']'
    
    # Use regex to extract all floating point numbers
    numbers = re.findall(r'-?\d+(?:\.\d+)?', bbox_string)
...
@click.option('-s', '--save_path',
              type=click.Path(file_okay=False,
                              writable=True,
                              path_type=Path),
              default=Path('../deep-text-edit/data/IMGUR5K'))
    ...
    for annotation_path, output_path in tqdm(zip(annotations, outputs)):
        ...
        for index_id, ann_ids in tqdm(annotations, leave=False):
            ...
            for ann_id in ann_ids:
                ...
                bounding_box = parse_bounding_box(info['bounding_box'])
                img_cropped = crop_minAreaRect(img, *bounding_box)
